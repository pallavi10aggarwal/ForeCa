...
class HomeViewController: UIViewController {
	...
	private func setupView() {
		...
		forecastTableView.register(
			UINib.init(nibName: NibFile.dayTableViewCell, bundle: nil),
			forCellReuseIdentifier: CellIdentifiers.dayTableViewCell
		)

		NotificationCenter.default.addObserver(
			self,
			selector: #selector(loadForecastData),
...
extension HomeViewController: UITableViewDataSource {
	...
	func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
		let identifier = CellIdentifiers.dayTableViewCell
		let cell = forecastTableView.dequeueReusableCell(withIdentifier: identifier) as? DayTableViewCell
